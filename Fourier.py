...
#			http://pybonacci.org/2012/08/30/manual-de-introduccion-a-matplotlib-pyplot-ix-miscelanea

import numpy.fft

def fft(signal,n=None,axis=-1,norm=None):
	#Este método está pensado como una interfaz entre la FFT de numpy y el usuario final del Matlab Framework
	#La función retorna una lista unidimensional que puede o no contener números complejos
	return numpy.ndarray.tolist(numpy.fft.fft(signal,n,axis,norm))


def ifft(signalTrf,n=None,axis=-1,norm=None):
	#Este método está pensado como una interfaz entre la IFFT de numpy y el usuario final del Matlab Framework
	#La función retorna una lista unidimensional que puede o no contener números complejos
	return numpy.ndarray.tolist(numpy.fft.ifft(signalTrf,n,axis,norm))


def fft2(matriz):
	#Esta función actua como un front-end para el método fft2 de numpy
	#retorna un arreglo de tipo lista de 2 dimensiones
	#para mostrarse se puede usar imshow() realizando previamente un mapeo de valores
	return numpy.ndarray.tolist(numpy.fft.fft2(matriz))

def ifft2(matriz):
	#Esta función actua como un front-end para el método ifft2 de numpy
	#retorna un arreglo de tipo lista de 2 dimensiones
	#para mostrarse se puede usar imshow() realizando previamente un mapeo de valores
	#teniendo en cuenta que no será como la original debido a problemas de precisión numérica
	return numpy.ndarray.tolist(numpy.fft.ifft2(matriz))
...
#								FASE ALPHA DE LA PROGRAMACIÓN DE LA FFT JULIO ECHEVERRI
